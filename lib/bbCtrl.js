    ...
    class bbController {
        ...
        run(options) {
            ...
            videoBackupChannel.watch(function(data) {
                ...
                } else if (data.type === "add-download-file") {
                    let line = data.fileName;
                    let filename = playlistFolder + "/videos.txt";
                    if (!fs.existsSync(playlistFolder)){
                        fs.mkdirSync(playlistFolder, {recursive: true});
                    }
                    fs.appendFileSync(filename, "file " + line + "\n", function(err) {
                        if(err) {
                            return console.log("error: ", err);
                        }
                    });
                } else if (data.type === 'download-last-line') {
                    let filename = playlistFolder + "/last-" + data.line;
                    fs.appendFileSync(filename, "0", function(err) {
                        if(err) {
                            return console.log("error: ", err);
                        }
                    });
