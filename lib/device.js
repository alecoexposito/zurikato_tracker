...
function Device(adapter, connection, gpsServer) {
    ...
    this.on('data', function(data) {
        ...
        console.log("message parts: ", msgParts);
    ...
    this.make_action = function(action, msgParts) {
        ...
        switch (action) {
            case 'login_request':
                ...
                break;
            case 'ping':
                _this.ping(msgParts, _this.getUID());
                break;
            case 'alert':
                _this.receive_alarm(msgParts, _this.getUID());
                break;
            case 'other':
    ...
    this.ping = function(msgParts, device_id) {
        var gpsData = this.adapter.get_ping_data(msgParts, device_id);
        if (gpsData === false) {
    ...
    this.receive_alarm = function(msgParts, device_id) {

        var alarmData = _this.adapter.receive_alarm(msgParts, device_id);
        console.log("sending alarm");
        _this.emit('alert', alarmData, alarmData, msgParts);
    };
